...
real eta = 0.0005; //默认0.05 Initial learning rate
real alpha = 0.75, x_max = 100.0; // 默认x_max = 100。Weighting function parameters, not extremely sensitive to corpus, though may need adjustment for very small or very large corpora
...
real *W, *gradsq, *cost, *gradsqmj=NULL,*gradsqm=NULL,*gradsqj=NULL;
...
void *glove_thread(void *vid) {
    ...
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        double diff = 0.0,diff0 = 0.0,d_a=0.0,d_b=0.0;
        for (b = 0; b < vector_size; b++) diff += W[b + l1] * W[b + l2];
        //diff = diff;
        //for (b = 0; b < vector_size; b++) {diff0 += W[b + l1] * W[b + l2]; d_a +=W[b + l1] * W[b + l1]; d_b +=W[b + l2] * W[b + l2]; }// dot product of word and context word vector  to   cosine similarity 
        //diff = diff0 / (sqrt(d_a)*sqrt(d_b)); 
        ...
        //fdiff = diff * cr.val / vector_size; //cosine similarity  multiply weighting function (f) with diff
        ...
        for (b = 0; b < vector_size; b++) {
            ...
            temp1 = fmin(fmax(fdiff * W[b + l2], -grad_clip_value), grad_clip_value);
            temp2 = fmin(fmax(fdiff * W[b + l1], -grad_clip_value), grad_clip_value);
            // adaptive updates
            //W_updates1[b] = temp1 / sqrt(gradsq[b + l1]);
            //W_updates2[b] = temp2 / sqrt(gradsq[b + l2]);
            //W_updates1[b] = temp1 * eta / sqrt(gradsq[b + l1]+e);//RMSprop法
            //W_updates2[b] = temp2 * eta/ sqrt(gradsq[b + l2]+e);//RMSprop法
            
            W_updates1[b] = gradsqmj[b + l1] * eta / (sqrt(gradsqj[b + l1])+e);//updata parameters
            W_updates2[b] = gradsqmj[b + l2] * eta / (sqrt(gradsqj[b + l2])+e);//updata parameters
            
            
            W_updates1_sum += W_updates1[b];
            W_updates2_sum += W_updates2[b];
            //W_updates1_sum = pl1*W_updates1_sum +(1-pl1)* W_updates1[b];
            //W_updates2_sum =pl2*W_updates2_sum + (1-pl2)* W_updates2[b];
            gradsqm[b + l1] = pl1*gradsqm[b + l1]+(1-pl1) * temp1;   //更新有偏一阶矩估计 类似Momentum法
            gradsqm[b + l2] = pl2*gradsqm[b + l2]+(1-pl1) * temp2;    //更新有偏一阶矩估计 类似Momentum法
            //gradsq[b + l1] += temp1 * temp1;
            gradsq[b + l1] = pl1*gradsq[b + l1]+(1-pl2) * temp1 * temp1;   //更新有偏二阶矩估计 RMSprop法
            //gradsq[b + l2] += temp2 * temp2;
            gradsq[b + l2] = pl2*gradsq[b + l2]+(1-pl2) * temp2 * temp2;    //更新有偏二阶矩估计 RMSprop法
            
            gradsqmj[b + l1] = gradsqm[b + l1] / (1-pow(pl1, t) );   //修正一阶矩的偏差：
            gradsqmj[b + l2] = gradsqm[b + l2] / (1-pow(pl1, t) );   //修正一阶矩的偏差：
            
            gradsqj[b + l1] = gradsqm[b + l1] / (1-pow(pl2, t) );   //修正二阶矩的偏差：
            gradsqj[b + l2] = gradsqm[b + l2] / (1-pow(pl2, t) );  //修正二阶矩的偏差：
