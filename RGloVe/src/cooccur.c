    ...
    while (1) {
        ...
        for (k = j - 1; k >= ( (j > window_size) ? j - window_size : 0 ); k--) { // Iterate over all words to the left of target word, but not past beginning of line
            ...
            if ( w1 < max_product/w2 ) { // Product is small enough to store in a full array
                bigram_table[lookup[w1-1] + w2 - 2] += distance_weighting ? 1.0/((real)(exp(-(j-k)))) : 1.0; // Weight by inverse of distance between words if needed
                if (symmetric > 0) bigram_table[lookup[w2-1] + w1 - 2] += distance_weighting ? 1.0/((real)(exp(-(j-k)))) : 1.0; // If symmetric context is used, exchange roles of w2 and w1 (ie look at right context too)
            }
                ...
                cr[ind].word1 = w1;
                cr[ind].word2 = w2;
                cr[ind].val = distance_weighting ? 1.0/((real)(exp(-(j-k)))) : 1.0;
